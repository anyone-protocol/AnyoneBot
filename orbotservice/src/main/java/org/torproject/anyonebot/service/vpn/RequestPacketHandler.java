    ...
    public void run() {
        try {
            ...
            if (dnsResp != null) {
                ...
                UdpPacket.Builder udpBuilder = new UdpPacket.Builder(udpPacket).srcPort(udpPacket.getHeader().getDstPort()).dstPort(udpPacket.getHeader().getSrcPort()).srcAddr(packet.getHeader().getDstAddr()).dstAddr(packet.getHeader().getSrcAddr()).correctChecksumAtBuild(true).correctLengthAtBuild(true).payloadBuilder(dnsBuilder);
                ...
                if (packet instanceof IpV4Packet ipPacket) {
                    ...
                    respPacket = new IpV6Packet.Builder((IpV6Packet) packet).srcAddr((Inet6Address) packet.getHeader().getDstAddr()).dstAddr((Inet6Address) packet.getHeader().getSrcAddr()).payloadBuilder(udpBuilder).build();
                }
