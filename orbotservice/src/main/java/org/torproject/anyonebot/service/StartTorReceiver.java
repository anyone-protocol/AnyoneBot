        ...
        try {
            ...
            if (TextUtils.equals(action, ACTION_START)) {
                ...
                if (Prefs.allowBackgroundStarts()) {
                    Intent startTorIntent = new Intent(context, AnyoneBotService.class).setAction(action).putExtra(AnyoneBotConstants.EXTRA_NOT_SYSTEM, true);
                    if (packageName != null) {
                        startTorIntent.putExtra(AnyoneBotService.EXTRA_PACKAGE_NAME, packageName);
                    }
