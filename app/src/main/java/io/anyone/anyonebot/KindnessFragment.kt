...
class KindnessFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        swVolunteerMode.setOnCheckedChangeListener { _, isChecked ->
            ...
            sendIntentToService(AnyoneBotConstants.CMD_SNOWFLAKE_PROXY)
        }

        view.findViewById<TextView>(R.id.swVolunteerAdjust).setOnClickListener {
            KindessConfigBottomSheet().show(requireActivity().supportFragmentManager,
    ...
    private fun sendIntentToService(action: String) {
        val intent = Intent(requireContext(), AnyoneBotService::class.java)
            .setAction(action)
            .putNotSystem()
