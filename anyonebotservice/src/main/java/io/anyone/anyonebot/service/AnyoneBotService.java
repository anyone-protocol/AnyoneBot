...
import io.anyone.anyonebot.service.util.CustomAnonResourceInstaller;
...
public class AnyoneBotService extends VpnService implements AnyoneBotConstants {
    ...
    private final static String NOTIFICATION_CHANNEL_ID = "anyonebot_channel_1";
    ...
    AnyoneBotRawEventListener mRawEventListener;
    AnyoneBotVpnManager mVpnManager;
    Handler mHandler;
    ...
    protected void showToolbarNotification(String notifyMsg, int notifyType, int icon) {
        ...
        var pendIntent = PendingIntent.getActivity(AnyoneBotService.this, 0, intent, PendingIntent.FLAG_IMMUTABLE);
    ...
    public IBinder onBind(Intent intent) {
        Log.d(TAG, "AnyoneBotService: onBind");
                ...
                case ACTION_START -> {
                    ...
                    if (Prefs.useVpn()) {
                        if (mVpnManager != null && (!mVpnManager.isStarted())) { // start VPN here
                            Intent vpnIntent = VpnService.prepare(AnyoneBotService.this);
    ...
    private void sendStatusToAnyoneBotActivity() {
        var localStatus = new Intent(LOCAL_ACTION_STATUS).putExtra(EXTRA_STATUS, mCurrentStatus);
        LocalBroadcastManager.getInstance(AnyoneBotService.this).sendBroadcast(localStatus); // update the activity with what's new
    }
}
